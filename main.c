...
void checkTestArrays()
{
	...
	{
		struct subArrIndices testSubArrIndices = closeToZero1(testArrays[i], TEST_ARRAY_SIZE);
		int subArr[testSubArrIndices.end - testSubArrIndices.start];
		for (int j = 0; testSubArrIndices.start + j <= testSubArrIndices.end; j++)
		{
			subArr[j] = testArrays[i][testSubArrIndices.start + j];
		}

		int sumOfSubArr = sumOfArray(subArr, testSubArrIndices.end - testSubArrIndices.start + 1);
